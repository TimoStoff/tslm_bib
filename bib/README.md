...
```sh
cp ../aionic_paper_guides/bib/all.bib .
```

For shared updates, add this repository as a Git submodule:

```sh
git submodule add https://github.com/TimoStoff/tslm_bib.git aionic_paper_guides
```

Then reference it from the paper:

```tex
\bibliography{aionic_paper_guides/bib/all}
```
